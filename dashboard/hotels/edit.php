...
//   echo $id;
$update = "SELECT * FROM hotels WHERE id=$id";
...
if (isset($_POST['update'])) {
    ...
    // if (file_exists('../img/HotelImages/' . $c_image)) {
    //     $filename = $_FILES['file']['name'];
    //     $_SESSION['status'] = 'image already Exist' . $filename;
    //     // header('Location:edit.php?id=<?=echo $id');
    // } else {
        $c_update = "update hotels set hotelName='$title', file_name='$update_c_image', location='$location', duration= '$time', price='$price', near= '$near'
	where id='$id'";
        ...
        if ($run_update) {
            ...
            if ($c_image != '') {
                move_uploaded_file($c_image_temp, "../img/HotelImages/$c_image");
                $trimed_path = trim('../img/HotelImages/' . $old_image);
                unlink($trimed_path);
            }

            $_SESSION['status'] = 'hotel update successful';
            ...
            <div class="row">
                ...
                <div class="col-sm-6 " style="padding-top: 10px;">
                    <div class="bg-light rounded h-100 p-4">
                        <h3 class="mb-4 text-primary">Update Hotel</h3>
                        ...
                        <form method='POST' action="" enctype="multipart/form-data">
                            ...
                            <div class="form-floating mb-3">
                                ...
                                <input type="text" class="form-control" id="floatingInput" placeholder="news title" name="hotelName" value=" <?= $user->hotelName; ?>">
                                <label for="floatingInput">Hotel Name</label>

                            </div>
                            <div class="form-floating mb-3">
                                <input type="file" class="form-control" placeholder="Attach file" name="file">
                                <input type="hidden" class="form-control" name="file_old" value="<?=$user->file_name; ?>">

                                <label>File</label>
                            </div>
                            <?php
                    // while($row = $query->fetch_assoc()){
       $imageURL = '../img/HotelImages/'.$user->file_name;
                    // }    
                            ?>
                     <div class="form-floating mb-3">
                        <img src="<?php echo $imageURL; ?>" width=250px  />
                            ...
                            <div class="form-floating mb-3">
                                <input type="text" class="form-control" id="floatingInput" placeholder="news title" name="location" value="<?= $user->location; ?>">
                                <label for="floatingInput">location</label>
                            </div>
                            <div class="form-floating mb-3">
                                <input type="text" class="form-control" id="floatingInputs" placeholder="published by" name="duration" value="<?= $user->duration; ?>">
                                <label for="floatingInput">Duration</label>
                            </div>
                            <div class="form-floating mb-3">
                                <input type="text" class="form-control" id="floatingInputss" placeholder="Date" name="nearby" value="<?= $user->near; ?>">
                                <label for="floatingInput">near</label>
                            </div>
                            <div class="form-floating mb-3">
                                <input type="text" class="form-control" id="floatingInputss" placeholder="price" name="price" value="<?= $user->price; ?>">
