...
//   echo $id;
$update = "SELECT * FROM blog WHERE id=$id";
...
if (isset($_POST['update'])) {
    ...
    // if (file_exists('../img/blog/' . $c_image)) {
    //     $filename = $_FILES['file']['name'];
    //     $_SESSION['status'] = 'image already Exist' . $filename;
    //     // header('Location:edit.php?id=<?=echo $id');
    // } else {
        $c_update = "update blog set title='$title', file_name='$update_c_image', description='$description'
	where id='$id'";
        ...
        if ($run_update) {
            ...
            if ($c_image != '') {
                move_uploaded_file($c_image_temp, "../img/blog/$c_image");
                $trimed_path = trim('../img/blog/' . $old_image);
                unlink($trimed_path);
            }

            $_SESSION['status'] = 'blog update successful';
...
<body>
    <div class="container-xxl position-relative bg-white d-flex p-0">
       ...
       $imageURL = '../img/blog/'.$user->file_name;
                    // }    
                            ?>
                     <div class="form-floating mb-3">
